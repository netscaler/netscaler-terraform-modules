...
resource "azurerm_network_security_rule" "terraform-allow-client-http-from-internet" {
  ...
  protocol                    = "Tcp"
  source_port_range           = "*"
  destination_port_ranges     = ["80", "443", "6379", "8443"]
  source_address_prefix       = "*"
  destination_address_prefix  = "*"
  resource_group_name         = azurerm_resource_group.terraform-resource-group.name
...
resource "azurerm_network_interface" "terraform-adc-management-interface" {
  ...
  # An additional IP configuration for SNIP with mgmt access.
  ip_configuration {
...
resource "azurerm_network_interface_backend_address_pool_association" "tf_assoc_mgmt" {
  network_interface_id    = element(azurerm_network_interface.terraform-adc-management-interface.*.id, count.index)
  ip_configuration_name   = "management-secondary"
  backend_address_pool_id = azurerm_lb_backend_address_pool.tf_backend_pool_mgmt.id

  count = 2
}

# LB Rule for management SNIPs
resource "azurerm_lb_rule" "allow_http_mgmt" {
  ...
  idle_timeout_in_minutes        = 4
  load_distribution              = "Default"
  probe_id                       = azurerm_lb_probe.tf_probe_mgmt.id
...
# Backend pool for SNIPs from management subnet
resource "azurerm_lb_backend_address_pool" "tf_backend_pool_mgmt" {
  loadbalancer_id = azurerm_lb.tf_lb_mgmt.id
  name            = "BackEndAddressPoolMgmt"
}

resource "azurerm_lb_probe" "tf_probe_mgmt" {
  loadbalancer_id     = azurerm_lb.tf_lb_mgmt.id
  name                = "http-probe"
  port                = 9000
  protocol            = "Tcp"
...
resource "azurerm_lb" "tf_lb_mgmt" {
  name                = "tf_lb_mgmt"
  location            = var.location
  resource_group_name = azurerm_resource_group.terraform-resource-group.name
  sku                 = "Standard"
