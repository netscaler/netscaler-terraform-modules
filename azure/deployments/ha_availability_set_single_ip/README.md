# Citrix ADC HA pair with availability set (Management access enabled)
...
* Two Citrix ADC instances configured in a High Availability (SNIP with management access enabled)
...
In addition to the primary NSIP, each ADC management interface has a secondary IP configuration 
that is configured as a SNIP (Subnet IP) with management access enabled. This special configuration 
provides several benefits:

* **High Availability Management**: The management SNIP allows NITRO API calls and management traffic 
  to be routed through the private management load balancer, ensuring requests always reach the active 
  primary node even during failover scenarios.

* **Internal Management Access**: These SNIPs provide an alternative management endpoint that remains 
  private within the Virtual Network, useful for automation and monitoring tools that operate within 
  the Azure environment.

* **Backend Pool Integration**: The management SNIPs are added to the backend pool of the private 
  management load balancer (`tf_lb_mgmt`), which uses health probes to detect the active node and 
  route management traffic accordingly.

This architecture ensures consistent management connectivity regardless of which ADC instance is 
currently the HA primary, eliminating the need to track individual NSIP addresses for management 
operations when using the private management load balancer frontend IP.
...
Two Azure Load Balancers are deployed in this configuration:

## Public Load Balancer (tf_lb)

The public load balancer handles incoming traffic to the HA pair through the client subnet interfaces.

* **Frontend**: Public IP address (`alb_public_ip` output)
* **Backend Pool**: Both ADC client interfaces
* **Probe**: TCP probe on port 9000 to monitor ADC health
* **Load Balancing Rule**: Port 80 traffic with floating IP enabled

The public load balancer automatically detects failover events and redirects traffic to the active primary node.

## Private Management Load Balancer (tf_lb_mgmt)

The private load balancer handles internal management traffic through the management subnet SNIPs.

* **Frontend**: Private IP address from management subnet (`alb_private_mgmt_ip` output)
* **Backend Pool**: Both ADC management secondary interfaces (management SNIPs)
* **Probe**: TCP probe on port 9000 to monitor ADC health
* **Load Balancing Rule**: Port 80 traffic with floating IP enabled

This ensures that management traffic is always routed to the active primary node within the Virtual Network.
