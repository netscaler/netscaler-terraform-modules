...
resource "null_resource" "makeiso" {
  provisioner "local-exec" {
    command = format("./process_userdata.py --nsip %s --netmask %s --gateway %s --output-dir %s", var.nsip, var.subnetmask, var.gw_ip, var.iso_output_dir)
  }
}
...
resource "vsphere_file" "upload_iso" {
  ...
  source_file = format("%s.iso", var.iso_output_dir)
  destination_file = format("%s/%s.iso", var.iso_destination_folder, var.iso_output_dir)
...
resource "vsphere_virtual_machine" "citrixVPX" {
  ...
  cdrom {
    datastore_id  = data.vsphere_datastore.datastore.id
    path = format("%s/%s.iso", var.iso_destination_folder, var.iso_output_dir)
  }
