...
class HTTPNitro():
    ...
    def do_get(self, resource, id=None, action=None, loop=7):
        ...
        for i in range(loop):
            try:
                ...
                logger.error('Request Timeout {}'.format(e))
                if(i<loop-1):
                    waitfor(2)
                    logger.info("Trying again")
        logger.error("Failed executing \"{}\". Returning now".format(url))
        return False

    def do_post(self, resource, data, id=None, action=None,loop=7):
        url = self.construct_url(resource, id, action)
    ...
    def do_put(self, resource, data, id=None, action=None,loop=1):
        url = self.construct_url(resource, id, action)
    ...
    def change_password(self, new_pass='nsroot'):
        ...
        if self.check_connection():
            ...
            if result:
                ...
                url_temp = "http://{}/nitro/v1/config/login".format(self.nsip)  # url_temp=self.construct_url(resource='login')
                headers_temp = {}
                headers_temp['Content-Type'] = 'application/json'
                data_temp = {"login":{"username":self.nsuser,"password":self.headers['X-NITRO-PASS'],"new_password":new_pass,}}
                logger.debug('POST {}'.format(url_temp))
                logger.debug('POST data: {}'.format(json.dumps(data_temp, indent=4)))
                logger.debug('HEADERS: {}'.format(json.dumps(headers_temp, indent=4)))

                r = requests.post(url=url_temp,headers=headers_temp,json=data_temp)
                response = r.json()
                logger.debug(response)
                if(r.status_code == 201 or r.status_code == 200):
                    self.nspass = new_pass
                    self.headers['X-NITRO-PASS'] = self.nspass
                    logger.info('Successfully changed password of {} to {} having force password change enabled'.format(self.nsip,new_pass))
                    return True
