...
class CitrixADC(HTTPNitro):
    ...
    def change_password(self, new_pass='nsroot'):
        ...
        logger.debug(
            'Before changing the password of {}, checking if the password is already updated'.format(self.nsip))
...
def add_rest_nodes_to_cluster(rest_nodeips, rest_nodeids, cluster_backplane='1/1', tunnelmode='GRE'):
    ...
    for i in range(len(rest_nodeips)):
        ...
        # TODO: need to take password as input
...
if __name__ == "__main__":
    ...
    if current_node_dict: 
        ...
        if operation == "delete": # delete node
            ...
            new_node_ips_to_add = list(set(node_ips) - set(current_node_ips))
            new_node_ids_to_add = range(len(current_node_ips), len(node_ips))
        ...
        if len(node_ips) > 1:
            # join other nodes
            node_ids = range(1, len(node_ips))
            if add_rest_nodes_to_cluster(node_ips[1:], node_ids, backplane, tunnelmode):
                logger.info(
